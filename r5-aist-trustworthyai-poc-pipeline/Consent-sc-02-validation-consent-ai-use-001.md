# Consent: AI Use for PoC Scenario sc-02-validation - v0.1.0

* [**Table of Contents**](toc.md)
* [**Artifacts Summary**](artifacts.md)
* **Consent: AI Use for PoC Scenario sc-02-validation**

## Example Consent: Consent: AI Use for PoC Scenario sc-02-validation

Profile: [EU AI & EHDS Consent Profile](StructureDefinition-eu-ai-consent.md)

## Participants

* **Role**: Patient
  * **Details**: [Anonymous Patient Female, DoB: 1959-04-12](Patient-patient-001.md)

* [Rule](https://hl7.org/fhir/R5/formats.html#table): permit
  * [What](https://hl7.org/fhir/R5/formats.html#table): 
  * [Rules](https://hl7.org/fhir/R5/formats.html#table)![](data:image/png;base64,): 
* [Rule](https://hl7.org/fhir/R5/formats.html#table): ![](icon-qi-hidden.png)deny
  * [What](https://hl7.org/fhir/R5/formats.html#table): * Purpose: [ActReason: RESCH](http://terminology.hl7.org/7.2.0/CodeSystem-v3-ActReason.html#v3-ActReason-RESCH) (research)

  * [Rules](https://hl7.org/fhir/R5/formats.html#table)![](data:image/png;base64,): 
* [Rule](https://hl7.org/fhir/R5/formats.html#table): ![](data:image/png;base64,) Documentation for this format



## Resource Content

```json
{
  "resourceType" : "Consent",
  "id" : "sc-02-validation-consent-ai-use-001",
  "meta" : {
    "profile" : ["http://example.org/fhir/eu-ai-transparency/StructureDefinition/eu-ai-consent"]
  },
  "extension" : [{
    "url" : "http://example.org/fhir/eu-ai-transparency/StructureDefinition/patient-ai-info-provided",
    "valueBoolean" : true
  }],
  "status" : "active",
  "category" : [{
    "coding" : [{
      "system" : "http://terminology.hl7.org/CodeSystem/consentcategorycodes",
      "code" : "npp",
      "display" : "Notice of Privacy Practices"
    }],
    "text" : "Notice of Privacy Practices"
  }],
  "subject" : {
    "reference" : "Patient/patient-001"
  },
  "date" : "2026-03-01",
  "decision" : "permit",
  "provision" : [{
    "purpose" : [{
      "system" : "http://terminology.hl7.org/CodeSystem/v3-ActReason",
      "code" : "RESCH",
      "display" : "research"
    }]
  }]
}

```
