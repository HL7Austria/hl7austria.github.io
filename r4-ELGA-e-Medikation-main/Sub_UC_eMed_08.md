...
Ist keine Anpassung des Medikationsplaneintrags erforderlich, führt der GDA ein GET auf das Collection Bundle aus, das den Medikationsplan mit allen zugehörigen relevanten Ressourcen enthält ([Read-only-Zugriff](interactions.md#read-only-zugriff)). Basierend auf vorhandenen Planeinträgen (MedicationRequests) erstellt der GDA neue Geplante Abgaben (MedicationRequest) wie folgt:
